...
void buscar_archivo(archivo_t* lista, char* nombre) {
    archivo_t* actual = lista;

    while (actual != NULL) {
        if (strcmp(actual->nombre, nombre) == 0) {
            printf("Archivo encontrado en el directorio %s.\n", actual->directorio);
            return;
        }
        actual = actual->siguiente;
    }
...
void listar_archivos(archivo_t* lista) {
    archivo_t* actual = lista;
    printf("\n\n~~~~~~~~~~~~ ARCHIVOS ~~~~~~~~~~~~~~\n");
    while (actual != NULL) {
        printf("Nombre: %s - Directorio: %s\n", actual->nombre, actual->directorio);
        actual = actual->siguiente;
    }
    printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
}

void options_menu(archivo_t* lista, char* nombre, char* directorio) {
    int opcion;
    do {
        printf("~~~~~~~~~~~~ MENÚ ~~~~~~~~~~~~~~\n");
        ...
        switch (opcion) {
            ...
            case 3:
                ...
                scanf("%s", nombre);
                buscar_archivo(lista, nombre);
                break;
            case 4:
                listar_archivos(lista);
                break;
            case 5:
...
int main() {
    ...
    options_menu(lista, nombre, directorio);
